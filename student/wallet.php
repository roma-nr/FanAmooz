...
if (is_post() && verify_csrf() && ($_POST['action'] ?? '') === 'withdraw') {
    ...
    if ($amount < 5000) {
        ...
        try {
            wallet_withdraw($userId, $amount, 'برداشت از کیف پول');
...
// دریافت ۲۰ تراکنش آخر
$txns = db()->prepare("SELECT * FROM wallet_transactions WHERE user_id = ? ORDER BY created_at DESC LIMIT 20");
$txns->execute([$userId]);
